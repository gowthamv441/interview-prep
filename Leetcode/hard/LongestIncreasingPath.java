...
One optimization is that we can use a set to prevent the repeat visit in one DFS search. This optimization will reduce the time complexity for each DFS to O(mn)O(mn)O(mn) and the total algorithm to O(m2n2)O(m^2n^2)O(m2n2).
