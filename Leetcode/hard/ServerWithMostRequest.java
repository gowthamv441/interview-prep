...
use min heap with size k. heap is sorted based on the end of the request 
when new req comes in if the start time greater than the end time of minheap top
    then remove the top element from heap 
    and assign new request to the removed server and push it to heap
    keep track request count by servers

return max requests servers in list
TC: O(n logK)
SC: O(k)
*/